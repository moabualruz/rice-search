...
const binaryName =
  process.platform === "win32"
    ? "rice-search-server.exe"
    : "rice-search-server";
const binaryPath = path.join(process.cwd(), "..", "..", "build", binaryName);
...
test.describe("CLI Server", () => {
  test("should start and accept connections", async () => {
    ...
    const server = spawn(
      binaryPath,
      ["--http-port", port.toString(), "--grpc-port", grpcPort.toString()],
      {
    ...
    const waitForServer = new Promise<void>((resolve, reject) => {
      ...
      server.stdout.on("data", (data) => {
        ...
        if (
          output.includes("Starting HTTP server") ||
