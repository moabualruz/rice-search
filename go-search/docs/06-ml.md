...
| Model | Purpose | Size (FP16) | Dimensions | Tokens |
|-------|---------|-------------|------------|--------|
| SPLADE | Sparse encoding | ~250MB | Variable | 512 |
| Jina Embed v3 | Dense embedding | ~600MB | 1536 | 8192 |
| Jina Reranker v2 | Cross-encoder | ~500MB | 1 (score) | 512 |
...
| Mode | Description | VRAM Required |
|------|-------------|---------------|
| `all` (default) | All models loaded | ~2GB (FP16) |
| `ondemand` | Load when needed | ~600MB peak |
| `lru` | Keep N most recent | Configurable |
...
│  └─────────┘ └─────────┘ └─────────┘       │
│                                             │
│  Total: ~1.35GB (FP16) / ~2.7GB (FP32)     │
└─────────────────────────────────────────────┘
```

Best for: Dedicated GPU, consistent latency.
...
Timeline:
[Embed request] → Load Embed (500ms) → Inference (50ms) → Keep loaded
[Sparse request] → Load Sparse (300ms) → Inference (30ms) → Keep loaded
... 60s idle ...
[Timeout] → Unload all models
```
...
LRU_SIZE=2:
- Embed loaded, Sparse loaded
- Rerank request → evict oldest (Embed) → load Rerank
```
...
```bash
# All models loaded (default)
GPU_LOAD_MODE=all

# Load on demand
GPU_LOAD_MODE=ondemand
GPU_UNLOAD_TIMEOUT=60s
...
# Max wait for batch (if not full)
EMBED_BATCH_TIMEOUT=50ms
