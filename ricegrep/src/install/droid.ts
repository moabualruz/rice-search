...
const PLUGIN_ROOT =
  ...
  path.resolve(__dirname, "../../dist/plugins/ricegrep");
const PLUGIN_HOOKS_DIR = path.join(PLUGIN_ROOT, "hooks");
const PLUGIN_SKILL_PATH = path.join(PLUGIN_ROOT, "skills", "ricegrep", "SKILL.md");
...
async function installPlugin() {
  ...
  const hooksDir = path.join(root, "hooks", "ricegrep");
  const skillsDir = path.join(root, "skills", "ricegrep");
  const settingsPath = path.join(root, "settings.json");

  const watchHook = readPluginAsset(
    path.join(PLUGIN_HOOKS_DIR, "ricegrep_watch.py"),
  );
  const killHook = readPluginAsset(
    path.join(PLUGIN_HOOKS_DIR, "ricegrep_watch_kill.py"),
  );
  const skillContent = readPluginAsset(PLUGIN_SKILL_PATH);

  const watchPy = path.join(hooksDir, "ricegrep_watch.py");
  const killPy = path.join(hooksDir, "ricegrep_watch_kill.py");
  ...
  console.log(
    `Installed the ricegrep hooks and skill for Factory Droid in ${root}`,
  );
}
...
async function uninstallPlugin() {
  ...
  if (fs.existsSync(hooksDir)) {
    ...
    console.log("Removed ricegrep hooks from Factory Droid");
  } else {
    console.log("No ricegrep hooks found for Factory Droid");
  }

  if (fs.existsSync(skillsDir)) {
    fs.rmSync(skillsDir, { recursive: true, force: true });
    console.log("Removed ricegrep skill from Factory Droid");
  } else {
    console.log("No ricegrep skill found for Factory Droid");
  }
          ...
          const filtered = hooks[event].filter(
            (entry) =>
              entry?.hooks?.[0]?.command !==
                `python3 "${path.join(hooksDir, "ricegrep_watch.py")}"` &&
              entry?.hooks?.[0]?.command !==
                `python3 "${path.join(hooksDir, "ricegrep_watch_kill.py")}"`,
          );
...
export const installDroid = new Command("install-droid")
  .description("Install the ricegrep hooks and skill for Factory Droid")
  .action(async () => {
    await installPlugin();
  });

export const uninstallDroid = new Command("uninstall-droid")
  .description("Uninstall the ricegrep hooks and skill for Factory Droid")
  .action(async () => {
