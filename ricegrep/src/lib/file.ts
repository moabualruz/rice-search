 ...
 * These are not recognized as binary and also can't be uploaded to Rice Search.
 */
...
export interface FileSystem {
  ...
  /**
   * Loads the ricegrepignore file for a directory
   */
  loadRicegrepignore(dirRoot: string): void;
}
...
export class NodeFileSystem implements FileSystem {
  ...
  *getFiles(dirRoot: string): Generator<string> {
  ...
  private getDirectoryIgnoreFilter(dir: string): ReturnType<typeof ignore> {
    ...
    // Load .ricegrepignore
    const ricegrepignorePath = path.join(dir, ".ricegrepignore");
    if (fs.existsSync(ricegrepignorePath)) {
      ig.add(fs.readFileSync(ricegrepignorePath, "utf8"));
    }
