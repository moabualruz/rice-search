...
setup() {
    ...
    ln -sf "$DIR/../dist/index.js" "$BATS_TMPDIR/bin/ricegrep"
    PATH="$BATS_TMPDIR/bin:$PATH"

    export RICEGREP_IS_TEST=1
    export RICEGREP_TEST_STORE_PATH="$BATS_TMPDIR/ricegrep-test-store.json"
    ...
    ricegrep search --sync test
}

teardown() {
    rm -f "$RICEGREP_TEST_STORE_PATH"
...
@test "Prints help" {
    ...
    assert_success
    assert_output --partial 'Usage: ricegrep'
    assert_output --partial 'Options:'
    assert_output --partial 'Commands:'
}

@test "Prints version" {
    run ricegrep --version
...
@test "Search with content" {
    run ricegrep search --content test
...
@test "Search with max count" {
    run ricegrep search --max-count 1 "Hello, world!"
...
@test "Search with max count 2" {
    run ricegrep search --max-count 2 "Hello, world!"
...
@test "Search with .gitignore" {
    rm "$BATS_TMPDIR/ricegrep-test-store.json"
...
@test "Search with .ricegrepignore" {
    rm "$BATS_TMPDIR/ricegrep-test-store.json"
    echo "*.txt" > "$BATS_TMPDIR/test-store/.ricegrepignore"
    run ricegrep search --sync test
...
@test "Search with .ricegrepignore recursive" {
    # A .ricegrepignore file in a subdirectory should be respected
    rm "$BATS_TMPDIR/ricegrep-test-store.json"
    mkdir -p "$BATS_TMPDIR/test-store/test-dir"
    echo "*.txt" > "$BATS_TMPDIR/test-store/test-dir/.ricegrepignore"
    echo "Hello, world!\nA fourth test." > "$BATS_TMPDIR/test-store/test-dir/test-4.txt"
    run ricegrep search --sync test
...
@test "Search with .ricegrepignore recursive and allow patterns" {
    # A .ricegrepignore file in a subdirectory should be respected
    rm "$BATS_TMPDIR/ricegrep-test-store.json"
    mkdir -p "$BATS_TMPDIR/test-store/test-dir"
    printf "*.txt\n!test-5.txt" > "$BATS_TMPDIR/test-store/test-dir/.ricegrepignore"
    echo "Hello, world!\nA fourth test." > "$BATS_TMPDIR/test-store/test-dir/test-4.txt"
...
@test "Search with path scoping" {
    ...
    # ricegrep search --sync will sync current dir.
    run ricegrep search --sync Hello

    # Search in sub
    run ricegrep search Hello sub
...
@test "Search non-existent path" {
    run ricegrep search Hello /does/not/exist
...
@test "Search with no rerank" {
    run ricegrep search -c --no-rerank test
...
@test "Handles large git repositories with >1MB ls-files output" {
    ...
    run ricegrep search --max-file-count 1000000 --sync "Content"
...
@test "Search sync deletes files from store that are not present locally" {
    rm "$BATS_TMPDIR/test-store/test-2.txt"
    run ricegrep search --sync --dry-run test
...
@test "Config maxFileSize skips large files (YAML)" {
    rm "$BATS_TMPDIR/ricegrep-test-store.json"
    echo 'maxFileSize: 50' > "$BATS_TMPDIR/test-store/.ricegreprc.yaml"
...
@test "Config file .ricegreprc.yml also works" {
    rm "$BATS_TMPDIR/ricegrep-test-store.json"
    echo 'maxFileSize: 50' > "$BATS_TMPDIR/test-store/.ricegreprc.yml"
...
@test "Config CLI flag overrides config file" {
    rm "$BATS_TMPDIR/ricegrep-test-store.json"
    echo 'maxFileSize: 1000000' > "$BATS_TMPDIR/test-store/.ricegreprc.yaml"

    echo "tiny" > "$BATS_TMPDIR/test-store/tiny.txt"
    dd if=/dev/zero bs=100 count=1 2>/dev/null | tr '\0' 'y' > "$BATS_TMPDIR/test-store/medium.txt"

    cd "$BATS_TMPDIR/test-store"
    run ricegrep watch --dry-run --max-file-size 50
...
@test "Config env variable overrides config file" {
    ...
    cd "$BATS_TMPDIR/test-store"
    export RICEGREP_MAX_FILE_SIZE=50
    run ricegrep watch --dry-run
    unset RICEGREP_MAX_FILE_SIZE
...
@test "Config maxFileCount fails when exceeded" {
    ...
    run ricegrep watch --dry-run --max-file-count 3
...
@test "Config maxFileCount succeeds when not exceeded" {
    ...
    run ricegrep watch --dry-run --max-file-count 5
...
@test "Config maxFileCount via YAML" {
    ...
    echo 'maxFileCount: 2' > ".ricegreprc.yaml"
    for i in {1..4}; do
        echo "file $i" > "file-$i.txt"
    done
...
@test "Config maxFileCount env variable" {
    ...
    export RICEGREP_MAX_FILE_COUNT=2
    run ricegrep watch --dry-run
    unset RICEGREP_MAX_FILE_COUNT
...
@test "Config maxFileCount CLI overrides env variable" {
    ...
    export RICEGREP_MAX_FILE_COUNT=100
    run ricegrep watch --dry-run --max-file-count 2
    unset RICEGREP_MAX_FILE_COUNT
